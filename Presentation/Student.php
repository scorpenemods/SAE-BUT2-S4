...
<section class="Menus" id="Menus">
    ...
    <div class="Contenus">
        ...
        <div class="Contenu <?php echo $activeSection == '3' ? 'Visible' : ''; ?>" id="content-3">
            ...
            <button id="showModalBtn">Demander un nouveau formulaire de pré-convention</button>
            ...
            <div id="modalContainer" style="display: none; position: fixed; top: 0; left: 0; width: 100%; height: 100%; background: rgba(0, 0, 0, 0.5);">
                <div style="position: absolute; top: 50%; left: 50%; transform: translate(-50%, -50%); background: white; padding: 20px; border-radius: 8px; width: 300px;">
                    <h3>Sélectionnez un tuteur entreprise</h3>
                    <label for="tutorSelect">Compte du tuteur :</label>
                    <select id="tutorSelect" style="width: 100%; margin-top: 10px;">
                        <option value="">-- Choisissez un tuteur entreprise --</option>
                        <option value="-1">Mon tuteur n'a pas encore de compte</option>
                        <?php
                        $mentors = $database->getAllMentor();
                        foreach ($mentors as $mentor) {
                            echo "<option value=\"" . htmlspecialchars($mentor['id']) . "\">"
                                . htmlspecialchars($mentor['nom'] . " " . $mentor['prenom'])
                                . "</option>";
                        }
                        ?>
                    </select>
                    <br>
                    <button id="validateBtn" style="margin-top: 10px;">Valider</button>
                    <button id="closeModalBtn" style="margin-top: 10px; background-color: #f44336; color: white;">Annuler</button>
                </div>
            </div>
            ...
            if (!$idPreAgreementForm == null) {
                echo '<a href="PreAgreementFormStudent.php?id=' . htmlspecialchars($idPreAgreementForm['id']) . '">Votre formulaire de préconvention</a>';
            }
            ?>
            ...
            <script>
                ...
                // Valide la sélection et redirige l'utilisateur
                validateBtn.addEventListener('click', () => {
                    const selectedTutor = tutorSelect.value;

                    if (!selectedTutor) {
                        alert('Veuillez sélectionner un tuteur entreprise avant de continuer.');
                        return;
                    }

                    // Redirection vers le formulaire avec le paramètre du tuteur sélectionné
                    window.location.href = `PreAgreementFormStudent.php?tutor=${encodeURIComponent(selectedTutor)}`;
                });
            </script>
